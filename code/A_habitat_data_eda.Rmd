...
ggplot(aes(x = site, y = perc_cover, fill = identity), data = patchScale4fig)+ theme_bw()+
  geom_bar(stat = "identity")+
...
ggplot(aes(x = site, y = Area, fill = Habitat), data = landscapeArea)+ theme_bw()+
  geom_bar(stat = "identity")+
...
# Trawl Metrics
ggplot(aes(x = site, y = total_abund_sum), data = ecometrics)+ theme_bw()+
  geom_bar(stat = "identity")+
  labs(x = "Site", y = "Total CPUE")+
  theme(axis.text.x = element_text(angle = 90))+
  annotate("text",x=ecometrics$site,y=ecometrics$total_abund_sum+30,label=ecometrics$comm_richness_sum)
...
ggplot(aes(x = deployment, y = temperature), data = tempNightAllSites)+theme_bw()+
  geom_boxplot()+
  labs(x = "Deployment", y = "Temperature")
